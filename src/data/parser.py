...
class ExpressionParser:
    def __init__(
        ...
        self._features = {f.name.lower(): f for f in Features}
        ...
        if top == "$":  # Feature next
            ...
            if self._pop_token() != "(":
                raise ExpressionParsingError(
                    '"Constant" should be followed by a left parenthesis'
                )
            value = self._to_float(self._pop_token())
            if self._pop_token() != ")":
                raise ExpressionParsingError(
                    '"Constant" should be closed by a right parenthesis'
                )
            ...
            if (
                not self._dollar_needed
                and (feature := self._features.get(top)) is not None
            ):
                return Feature(feature)
                ...
                raise ExpressionParsingError(
                    f"Cannot find the operator/feature name {top}"
                )
    ...
    def _process_punctuation(self) -> None:
        if len(self._tokens) == 0:
            return
        top = self._pop_token()
        stack_top_is_ops = len(self._stack) != 0 and not isinstance(
            self._stack[-1], Expression
        )
    ...
    def _build_one_subexpr(self) -> None:
        if (
            op_idx := find_last_if(self._stack, lambda item: isinstance(item, list))
        ) == -1:
        ...
        if any(not isinstance(item, Expression) for item in operands):
            raise ExpressionParsingError(
                "An operator name cannot be used as an operand"
            )
        operands = cast(List[Expression], operands)
        dt_operands = operands
        if (
            not self._suffix_needed
            and isinstance(operands[-1], Constant)
            and (dt := self._as_delta_time(operands[-1], noexcept=True)) is not None
        ):
    ...
    @overload
    def _as_delta_time(
        self, value: _DTLike, noexcept: Literal[False] = False
    ) -> DeltaTime: ...
    @overload
    def _as_delta_time(
        self, value: _DTLike, noexcept: Literal[True]
    ) -> Optional[DeltaTime]: ...
    @overload
    ...
    def _as_delta_time(self, value: _DTLike, noexcept: bool = False):
        ...
        if int(value) <= 0 and not self._allow_np_dt:
            maybe_raise(
                f"A DeltaTime should refer to a positive time difference, but got {int(value)}d"
            )
            return

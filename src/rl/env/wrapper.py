...
class AlphaEnvWrapper(gym.Wrapper):
    ...
    def action_masks(self) -> np.ndarray:
        res = np.zeros(self.size_action, dtype=bool)  # 标记符或者值的位置，进行mask
    ...
    def action_to_token(self, action: int) -> Token:
        ...
        if action < SIZE_FEATURE:
            return FeatureToken(Features(action))
        action -= SIZE_FEATURE
...
def AlphaEnv(
    pool: AlphaPoolBase, subexprs: Optional[List[Expression]] = None, **kwargs
):
